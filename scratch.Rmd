...
roll_S_I_len_yrs = 4 * obs_period_len
roll_A_D_len_yrs = 2 * obs_period_len
...
inc_mort = obs %>% group_by(.id) %>%
  group_modify(~ .x %>%
                 transmute(roll_S_I = c(rep(NA, roll_S_I_len_obs-1),
                                        rollsum(.x %>% select(S_I),
                                                roll_S_I_len_obs)),
                           # this indexing ensures period in which mortality is
                           # counted doesn't overlap with period in which past
                           # incidence enters counted immunomodulated population
                           roll_A_D = c(rollsum(.x %>% select(A_D),
                                                roll_A_D_len_obs)[-1],
...
r2s = inc_mort %>%
  group_by(.id) %>%
  group_map(~ cor(.x$roll_S_I, .x$roll_A_D)^2) %>% unlist()

# Scatter plot immunomodulated and non-measles mortality populations.
inc_mort %>% ggplot(aes(x=roll_S_I, y=roll_A_D)) +
  geom_point(size=0.01) +
  facet_wrap(~.id,ncol=3, labeller = as_labeller(function(id) {
    paste0("simulation ", id, ", r^2=", signif(r2s[as.integer(id)], 2)) })) +
  xlab(paste0(signif(roll_S_I_len_yrs, 2), "-yr past mz incidence")) +
  ylab(paste0(signif(roll_A_D_len_yrs, 2), "-yr future non-mz mortality"))
```