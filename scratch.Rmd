...
compute_inc_mort_r2s = function(obs, roll_S_I_len_yrs, roll_A_D_len_yrs) {
  ...
  r2s = inc_mort %>%
    group_by(.id) %>%
    group_map(~ cor(.x$mz_immunomodulated, .x$non_mz_mort)^2) %>% unlist()
  return(list(inc_mort=inc_mort, r2s=r2s))
}

roll_S_I_lens_yrs = 1:16 * obs_period_len
roll_A_D_len_yrs = 4 * obs_period_len
...
inc_mort_r2s[[12]]$inc_mort %>% ggplot(aes(x=mz_immunomodulated, y=non_mz_mort)) +
  geom_point(size=0.1) +
  facet_wrap(~.id,ncol=3, labeller = as_labeller(function(id) {
    paste0("simulation ", id, ", r^2=",
           signif(inc_mort_r2s[[12]]$r2s[as.integer(id)], 2)) })) +
  xlab(paste0(signif(roll_S_I_lens_yrs[12], 2), "-yr mz incidence")) +
  ylab(paste0("following ", signif(roll_A_D_len_yrs, 2), "-yr non-mz mortality"))
```

```{r}
# Plot mz incidence-non mz mortality correlations vs. time periods covering
# immunomodulated populations.
r2s = bind_rows(inc_mort_r2s %>% purrr::map('r2s')) %>%
  tidyr::gather(key = "roll_S_I_len_yrs", value = "r2") %>%
  mutate(roll_S_I_len_yrs = as.numeric(roll_S_I_len_yrs))
r2_medians = r2s %>% group_by(roll_S_I_len_yrs) %>%
  summarise(r2 = median(r2))

ggplot(mapping = aes(x=roll_S_I_len_yrs, y=r2)) +
